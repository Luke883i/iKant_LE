...
function mutate(c,row){const n=row.node,f=row.f;c.mask|=bit(n);switch(n){
 ...
 case'DEADLINE': c.deadline_ok=false;break;
...
function oracleSession(c){if(c.exited)return'EXITED';if(c.preaccept_breach)return'SESSION_NONCONFORMING';if(c.integrity_codes.length)return'BLOCKED_INTEGRITY';if(!c.accepted)return c.terms_presented?'AWAITING_ACCEPTANCE':'ORIENTING';const unavailable=[c.evidence_available,c.byte_bridge,c.local_root,c.host_probe,c.writer,c.deadline_ok,c.active_commit].some(x=>x===false);if(unavailable)return'DEGRADED';const unknown=[c.evidence_available,c.byte_bridge,c.local_root,c.host_probe,c.writer,c.deadline_ok,c.active_commit,c.source_bound,c.consent_valid,c.transfer_identity].some(x=>x!==true);if(unknown)return'ACTIVATING';return'ACTIVE';}

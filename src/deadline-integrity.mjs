...
export function canResumeActivation(state){return Boolean(state?.accepted)&&state?.admission?.acceptance_consumed===true&&typeof state?.epoch==='string'&&state.epoch.length>0&&state?.status==='DEGRADED'&&state?.admission?.breached!==true&&state?.admission?.new_chat_required!==true&&!isDeadlineTerminal(state?.bootstrap?.deadline_terminal)&&typeof state?.admission?.acceptance_event_id==='string'&&state?.bootstrap?.acceptance_event_id===state.admission.acceptance_event_id&&HEX64.test(String(state?.bootstrap?.deadline_origin_receipt_sha256||''))&&HEX64.test(String(state?.bootstrap?.remote_history_sha256||''))&&Number.isFinite(state?.bootstrap?.deadline_elapsed_ms)&&state.bootstrap.deadline_elapsed_ms>=0;}

...
function zipStore(entries) {
  ...
  for (const [name, dataRaw] of entries) {
    ...
    const local = Buffer.concat([
      u32(0x04034b50), u16(20), u16(0x0800), u16(0), u16(0), u16(0), u32(crc), u32(data.length), u32(data.length), u16(nameBuf.length), u16(0), nameBuf, data
    ]);
    const central = Buffer.concat([
      u32(0x02014b50), u16(20), u16(20), u16(0x0800), u16(0), u16(0), u16(0), u32(crc), u32(data.length), u32(data.length), u16(nameBuf.length), u16(0), u16(0), u16(0), u16(0), u32(0), u32(offset), nameBuf
    ]);
...
export function buildBacklogDocx(model) {
  const body = [];
  body.push(paragraph(model.title, 'Title'));
  body.push(paragraph('TRACE/TELEMETRY; NOT INDEPENDENT EVIDENCE; NO PRIVATE CHAIN-OF-THOUGHT.'));
  body.push(paragraph(`Cycle: ${model.cycle}`));
  body.push(paragraph(`Terminal: ${model.terminal}`));
  for (const section of model.sections) {
    body.push(paragraph(section.title, 'Heading1'));
    for (const item of section.items) body.push(paragraph(`• ${item}`));
  }
...
export function writeBacklogDocx(filePath, model) {
  const bytes = buildBacklogDocx(model);
  fs.writeFileSync(filePath, bytes, { mode: 0o600 });
  const reread = fs.readFileSync(filePath);
